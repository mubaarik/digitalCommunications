...
import math, numpy, random, time, sys, wx
...
class WirelessNode:
    ...
    def on_xmit_success(self,packet):
        ...
        self.on_success(packet)

    def on_xmit_collision(self,packet):
    ...
    # is not complete.
    def step(self,time, do_output=True):
    ...
    # otherwise.
    def channel_access(self,time,ptime):
        return True
...
#		print 'attaching ', nextrate, ' to node ', n.get_id()
    ...
    def step(self,count=1,do_output=True):
	...
	self.stats._print(self.time, self.config.ptime, 'net')
        print "Fairness: %.2f" % self.fairness(0)
...
class NetFrame(wx.Frame):
	...
	psizer.Add(wx.StaticText(parameters,-1,'Backoff'),
		   0,wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL)
	backoff_choices = ['None','Binexpo'] # have to be strings
...
class Stats:
	...
	if type == 'net':
            print "Utilization: %.2f (%d out of %d attempts succeeded)" % (u, self.success, self.attempts)
	else:
...
def plot_data(wnet):
    ...
    for node in wnet.nlist:
        ...
        y_successes = y_successes + [node.get_id()] * len(node.sent)
        x_successes = x_successes + node.sent
        y_collisions = y_collisions + [node.get_id()] * len(node.coll)
        x_collisions = x_collisions + node.coll
    ...
    if x_successes != [] and y_successes != []:
        p.scatter(x_successes, y_successes, c='b', edgecolor='b', marker='s', alpha=.5, s=50)

    if x_collisions != [] and y_collisions != []:
        p.scatter(x_collisions, y_collisions, c='r', edgecolor="r", marker="s", alpha=.5, s=50)
    ...
    width = 0.35
    p.bar(ind, succ, width, color = 'g', edgecolor='g', alpha=.5)
    p.grid()
    p.ylabel("Number of Successful Receptions")
    p.xlabel("Node")
